...
3. **Run bracket simulation** for a year (e.g. 2025) to get win probabilities by round:
   ...
   ```bash
   python -m src.simulate       # 2025, 2000 sims by default
   ```
   ...
   out = run_simulation(year=2025, n_sims=5000)
...
- **Features:** Team-season stats (Barttorvik Neutral, Resumes, 538) joined by year + team; seed and round context. See **docs/PREDICTION_FEATURES.md** for what is required at prediction time for a new tournament year.
